...
- 状态：阶段 0–5 的最小接口实现完成；阶段 6 的跨环境、跨 seed 行为证据矩阵仍待执行。
- 实现快照（2026-09-20）：`--brain integrated` 已显式组合 PN/LH、MB 与 CX；默认仍为可比较的 legacy FSM。
...
| 阶段 | 名称 | 目标 | 依赖 | 退出准则 |
|---|---|---|---|---|
| 0 | 解释性基线 | 已实现核心距离、学习语义与聚合遥测 | 无 | 多模式运行字段统一；完整性能矩阵待测 |
| 1 | 脑—动作接口 | 已实现 `ActionCommand` / `MotorFeedback` 适配 | 阶段 0 | 旧控制器可保留为基线；统一竞争在集成模式启用 |
| 2 | 感觉编码与早期回路 | 已实现 ORN/AL/PN/LH、噪声罗盘和消融 | 阶段 1 | MB 已读取 PN 化学投射而非原始化学值 |
| 3 | CX 闭环导航 | 已实现观测罗盘、实际运动反馈、CX→motor | 阶段 1、2 | 遮挡 benchmark 与 `cx_motor` 消融可执行 |
| 4 | MB 价值学习 | 已实现双通道 DA、RPE、资格迹与 MBON 读出 | 阶段 1、2 | acquisition/reversal/blocked 专项行为任务仍待补全 |
| 5 | 社会感觉与动作反馈 | 已实现串行接触事件与 CPG/碰撞反馈 | 阶段 1 | 跨条件招募证据矩阵仍待执行 |
| 6 | 集成评估 | `--brain integrated`、演化接线、GUI 与确定性回归已完成 | 阶段 0–5 | 跨环境、5 seed 配对和性能门槛尚未完成 |

每阶段结束都可停止，不要求完成后续阶段才发布当前结果。当前集成模式只证明接口可运行和确定性，不构成跨 seed 的机制有效性或生物学拟合结论。
